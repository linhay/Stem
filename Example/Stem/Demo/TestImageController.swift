...
class TestImageController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        items.append(TableElement(title: "图像处理: 裁圆", subtitle: "image.st.rounded()") {[weak self] in
            guard let base = self else { return }
            base.imageView.image = base.image.st.rounded()
        })
        
        items.append(TableElement(title: "获取指定颜色的图片", subtitle: "UIImage(color: UIColor,size: CGSize)") { [weak self] in
            guard let base = self else { return }
            let image = UIImage(color: UIColor.red, size: CGSize(width: 50, height: 50))
            base.imageView.image = image
        })
        
        items.append(TableElement(title: "裁剪对应区域", subtitle: "image.st.crop(bound: CGRect)") { [weak self] in
            guard let base = self else { return }
        ...
        items.append(TableElement(title: "图像处理: 圆角", subtitle: "image.st.round(...)") {[weak self] in
            guard let base = self else { return }
        ...
        items.append(TableElement(title: "缩放至指定宽度", subtitle: "image.st.scaled(toWidth: CGFloat)") {[weak self] in
            guard let base = self else { return }
            let image = base.image.st.scaled(toWidth: 50)
            base.imageView.image = image
        })
        
        items.append(TableElement(title: "缩放至指定高度", subtitle: "image.st.scaled(toHeight: CGFloat)") {[weak self] in
            guard let base = self else { return }
